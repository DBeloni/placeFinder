...
function renderizarResultados(listaDeLocais) {
    ...
    if (listaDeLocais.length === 0) {
        container.innerHTML = '<p style="text-align:center; opacity:0.6; font-size:0.9rem; margin-top:20px;">Nenhum local encontrado para este raio.</p>';
        return;
    }
    ...
    listaDeLocais.forEach((local, index) => {
        ...
        card.setAttribute('endereco', `📍 A ${textoDistancia} de você`);
        card.setAttribute('link-mapa', '#');

        card.addEventListener('click', () => {
            map.flyTo([local.lat, local.lon], 17);
        });
